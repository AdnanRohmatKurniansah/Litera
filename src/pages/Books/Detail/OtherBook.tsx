...
import { useOtherBooks } from '@/api/queries/book'
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious } from '@/components/ui/carousel'

const OtherBook = ({ bookId }: { bookId: string }) => {
  const { data, isLoading } = useOtherBooks({ bookId, limit: 30 })
        ...
        <div className="grid grid-cols-3 md:grid-cols-3 gap-4 items-center">
          <div className="section-title relative col-span-2 mb-0 md:mb-2">
            <div className="title relative flex justify-start items-center mb-1">
              ...
              <h6 className="font-semibold text-primary text-[12px] md:text-[14px] tracking-wide uppercase">You May Also Like</h6>
            </div>
            <div className="heading">
              <h3 className="font-bold text-[18px] md:text-2xl">Explore other books you might enjoy</h3>
            </div>
          </div>
        </div>
