...
import { useBooksDiscounted } from '@/api/queries/book'
import { Carousel, CarouselContent, CarouselItem, CarouselNext, CarouselPrevious } from '@/components/ui/carousel'

const DiscountedBook = () => {
  const { data, isLoading } = useBooksDiscounted()
        ...
        <div className="grid grid-cols-3 md:grid-cols-3 gap-4 items-center">
          <div className="section-title relative col-span-2 mb-0 md:mb-2">
            <div className="title relative flex justify-start items-center mb-1">
              ...
              <h6 className="font-semibold text-primary text-[12px] md:text-[14px] tracking-wide uppercase">Limited-Time Deals</h6>
            </div>
            <div className="heading">
              <h3 className="font-bold text-[18px] md:text-2xl">Grab your favorite books at lower prices.</h3>
            </div>
          </div>
          <div className="view-all flex justify-end">
            <Button variant={'outline'} className="px-3 md:px-5 py-2 md:py-4">
              <Link className="flex items-center text-[13px] md:text-normal" to="/books?sortBy=price_low">
