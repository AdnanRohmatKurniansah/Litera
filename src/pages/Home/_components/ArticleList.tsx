        ...
        <div className="grid grid-cols-3 md:grid-cols-3 gap-4 items-center">
          <div className="section-title relative col-span-2 mb-0 md:mb-2">
            <div className="title relative flex justify-start items-center mb-1">
              ...
              <h6 className="font-semibold text-primary text-[12px] md:text-[14px] tracking-wide uppercase">Insights & Book Stories</h6>
            </div>
            <div className="heading">
              <h3 className="font-bold text-[18px] md:text-2xl">Tips, reviews, and reading inspiration</h3>
            </div>
          </div>
          <div className="view-all flex justify-end">
            <Button variant={'outline'} className="px-3 md:px-5 py-2 md:py-4">
              <Link className="flex items-center text-[13px] md:text-normal" to="/articles">
          ...
          <Carousel opts={{ align: "start" }}className="w-full">
            <CarouselContent className='ps-2 py-2'>
              ...
              ? Array.from({ length: 4 }).map((_, i) => (
                <CarouselItem key={i} className="basis-1/1 sm:basis-1/3 md:basis-1/4 lg:basis-1/4 px-2">
                  <ArticleCardSkeleton />
                </CarouselItem>
                ))
              : articles.map((article: Article, i: number) => (
                <CarouselItem key={i} className="group basis-1/1 sm:basis-1/3 md:basis-1/4 lg:basis-1/4 px-1 md:px-2">
